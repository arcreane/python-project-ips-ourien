...
class RadarWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # -----------------------------
        #  PISTE D’ATTERRISSAGE (réduite)
        # -----------------------------
        piste_longueur = radius * 0.065   # 6,5% du radar = 10x plus petit
        piste_largeur = 8                  # fine

        angle_deg = 35                     # inclinaison
        angle = math.radians(angle_deg)

        dx = piste_longueur * math.cos(angle)
        dy = piste_longueur * math.sin(angle)

        # Ligne principale
        pen_piste = QPen(QColor(230, 230, 230), piste_largeur)
        pen_piste.setCapStyle(Qt.RoundCap)
        painter.setPen(pen_piste)
        ...
        # Marquages (traits transversaux)
        pen_mark = QPen(QColor(255, 255, 255), 3)
        painter.setPen(pen_mark)

        for offset in range(-int(piste_longueur*0.8), int(piste_longueur*0.8), 40):
            px = center.x() + offset * math.cos(angle)
            py = center.y() - offset * math.sin(angle)

            painter.drawLine(px - 10, py - 10, px + 10, py + 10)

        # Boule / tour de contrôle
        painter.setPen(Qt.NoPen)
        painter.setBrush(QColor("#22C5FF"))
        painter.drawEllipse(center, 14, 14)

        # Image de la tour (si tu veux la remettre)
